			...
			if level == 0 {
				func(tk int64, tv *_timer_event) {
					...
					if checkValidTimer(tv.Id) {
						tv.ch <- tv.Id
						// logger.LogDebug("trigger timer ok, timerid:%d,ptid:%d,timeout:%d", tv.Id, tk, tv.Timeout)
					} else {
						// logger.LogWarn("trigger timer fail, timerid:%d,ptid:%d,timeout:%d", tv.Id, tk, tv.Timeout)
					}
				}(k, v)
...
func Add(id int64, timeout int64, ch chan int64) int64 {
	...
	// logger.LogDebug("add timer ok, timerid:%d,ptid:%d,timeout:%d", id, _timer_id, timeout)
	return _timer_id
}
